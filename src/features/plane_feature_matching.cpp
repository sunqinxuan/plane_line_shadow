...
#
# Last modified:	2019-05-17 14:56
#
...
//		leaf_max_interp=root;
	...
	bool PlaneFeatureMatching::match(Scan *scan, boost::shared_ptr<pcl::visualization::PCLVisualizer> vis)
	{
...
//			return false;
		...
		for(int i=0;i<scan->plane_matches.size();i++)
		{
			...
			Eigen::Vector4d pi;
			pi.block<3,1>(0,0)=scan->plane_matches[i].cur->normal;
			pi(3)=scan->plane_matches[i].cur->d;
			pi=Trc.getPlaneTransform()*pi;
			scan->plane_matches[i].ref->normal=pi.block<3,1>(0,0);
