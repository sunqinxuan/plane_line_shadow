...
//									*allplane = *plane + *allplane;
						...
						if(have_same_plane == false)
						{
							...
							tmp_plane->id=scan->observed_planes.size();
...
//							*allplane = *plane + *allplane;
	...
	{
//		std::ofstream fp_cov;
//		fp_cov.open("plane_cov.txt",std::ios::app);
		Eigen::Vector3d tmp;
		if(plane->points.size()>0)
		{
			...
			Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> es(plane->cov_xyz);
			plane->cov_eigenvalues=es.eigenvalues();
			plane->cov_eigenvectors=es.eigenvectors();
//			fp_cov<<"**********************************************"<<std::endl;
//			fp_cov<<plane->cov_eigenvalues.transpose()<<std::endl;
//			fp_cov<<plane->cov_eigenvectors<<std::endl<<std::endl;

			double tmp;
			Eigen::Vector3d tmp_vec;
			if(plane->cov_eigenvalues(0)>plane->cov_eigenvalues(1))
			{
				tmp=plane->cov_eigenvalues(0);
				plane->cov_eigenvalues(0)=plane->cov_eigenvalues(1);
				plane->cov_eigenvalues(1)=tmp;
				tmp_vec=plane->cov_eigenvectors.block<3,1>(0,0);
				plane->cov_eigenvectors.block<3,1>(0,0)=plane->cov_eigenvectors.block<3,1>(0,1);
				plane->cov_eigenvectors.block<3,1>(0,1)=tmp_vec;
			}
			if(plane->cov_eigenvalues(0)>plane->cov_eigenvalues(2))
			{
				tmp=plane->cov_eigenvalues(0);
				plane->cov_eigenvalues(0)=plane->cov_eigenvalues(2);
				plane->cov_eigenvalues(2)=tmp;
				tmp_vec=plane->cov_eigenvectors.block<3,1>(0,0);
				plane->cov_eigenvectors.block<3,1>(0,0)=plane->cov_eigenvectors.block<3,1>(0,2);
				plane->cov_eigenvectors.block<3,1>(0,2)=tmp_vec;
			}
			if(plane->cov_eigenvalues(1)>plane->cov_eigenvalues(2))
			{
				tmp=plane->cov_eigenvalues(1);
				plane->cov_eigenvalues(1)=plane->cov_eigenvalues(2);
				plane->cov_eigenvalues(2)=tmp;
				tmp_vec=plane->cov_eigenvectors.block<3,1>(0,1);
				plane->cov_eigenvectors.block<3,1>(0,1)=plane->cov_eigenvectors.block<3,1>(0,2);
				plane->cov_eigenvectors.block<3,1>(0,2)=tmp_vec;
			}
//			fp_cov<<plane->cov_eigenvalues.transpose()<<std::endl;
//			fp_cov<<plane->cov_eigenvectors<<std::endl<<std::endl;
...
//		fp_cov.close();
	...
	void visPlanes(Scan *scan, boost::shared_ptr<pcl::visualization::PCLVisualizer> vis)
	{
		...
		for(size_t i=0;i<scan->observed_planes.size();i++)
		{
			...
			pcl::visualization::PointCloudColorHandlerCustom <pcl::PointXYZRGBA> color1 (plane, red[i%14], grn[i%14], blu[i%14]);
