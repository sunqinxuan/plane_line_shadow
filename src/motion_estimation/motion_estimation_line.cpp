...
#
# Last modified:	2019-04-28 10:06
#
# Filename:		motion_estimation_line.cpp
#
# Description: 
#
===============================================*/
...
{
	/*
	void displayPlaneLineBA(Scan *scan, PlaneLineBA *plane_line_ba, boost::shared_ptr<pcl::visualization::PCLVisualizer> vis)
	{
		char id[20];
		...
		plane->resize(plane_line_ba->points_plane.size());
		for(int i=0;i<plane_line_ba->points_plane.size();i++)
		{
			plane->at(i).x=plane_line_ba->points_plane[i](0);
			plane->at(i).y=plane_line_ba->points_plane[i](1);
			plane->at(i).z=plane_line_ba->points_plane[i](2);
			plane->at(i).r=0;
			plane->at(i).g=0;
			plane->at(i).b=255;
		}
		...
		pcl::PointCloud<pcl::PointXYZRGBA>::Ptr line (new pcl::PointCloud<pcl::PointXYZRGBA>);
		line->resize(plane_line_ba->points_line.size());
		for(int i=0;i<plane_line_ba->points_line.size();i++)
		{
			line->at(i).x=plane_line_ba->points_line[i](0);
			line->at(i).y=plane_line_ba->points_line[i](1);
			line->at(i).z=plane_line_ba->points_line[i](2);
			line->at(i).r=0;
			line->at(i).g=255;
			line->at(i).b=0;
		}
		sprintf(id,"line");
		if (!vis->updatePointCloud (line, id))
			vis->addPointCloud (line, id);
		vis->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, id);

		pcl::PointCloud<pcl::PointXYZRGBA>::Ptr shadow (new pcl::PointCloud<pcl::PointXYZRGBA>);
		shadow->resize(plane_line_ba->points_shadow.size());
		for(int i=0;i<plane_line_ba->points_shadow.size();i++)
		{
			shadow->at(i).x=plane_line_ba->points_shadow[i](0);
			shadow->at(i).y=plane_line_ba->points_shadow[i](1);
			shadow->at(i).z=plane_line_ba->points_shadow[i](2);
			shadow->at(i).r=255;
			shadow->at(i).g=0;
			shadow->at(i).b=0;
		}
		sprintf(id,"shadow");
		if (!vis->updatePointCloud (shadow, id))
			vis->addPointCloud (shadow, id);
		vis->setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 5, id);
...
//		remove("plane_line_ba.txt");
		...
		for(int i=idx_camera_initial;i<map->cameras.size();i++)
		{
			for(size_t j=0;j<map->observ_planes.size();j++)
			{
				...
				{
					if(use_plane_) num_observ_planes_++;
			...
			for(size_t j=0;j<map->observ_lines.size();j++)
			{
				...
				{
					if(use_line_) num_observ_lines_++;
		...
		for(int i=idx_camera_initial;i<map->cameras.size();i++)
		{
			Eigen::Quaterniond quat=map->cameras[i].Quat();
			*cursor_param++=quat.w();
			*cursor_param++=quat.x();
			*cursor_param++=quat.y();
			*cursor_param++=quat.z();
			*cursor_param++=map->cameras[i].t(0);
			*cursor_param++=map->cameras[i].t(1);
			*cursor_param++=map->cameras[i].t(2);
		}
	...
	void PlaneLineBA::computePsi(const int &idx_camera, Matrix6d &Psi)
	{
		...
		for(size_t i=0;i<plane_line_ba->num_observ_shadows();i++)
		{
			if(shadow_camera_index()[i]==idx_camera)
			{
				...
				cursor_observ_shadows+=line_block_size;
				Matrix6d information(cursor_sqrt_information_shadows);
				cursor_sqrt_information_shadows+=line_block_size*line_block_size;
				...
				Eigen::Vector3d vv=-Transform::skew_sym(n)*Transform::skew_sym(v)*t_gc+skew_sym(u)*n;
				vv_norm=vv.norm();
				uu=uu/vv_norm;
				vv.transpose();
...
//			fp<<scans[0]->plane_matches[j].ref->normal.transpose()<<"\t"<<scans[0]->plane_matches[j].ref->d<<std::endl;
//			fp<<scans[0]->scan_ref->Tcg.inv().getMatrix()<<std::endl;
			...
			fp<<"\tobserv_planes  "<<plane_lm->planes[0]->normal.transpose()<<std::endl;
			plane_lm->id=plane_map_.size();
			plane_map_.push_back(plane_lm);

			*cursor_feature++=plane_lm->id;
			*cursor_camera++=0;
			*cursor++=scans[0]->plane_matches[j].ref->normal(0);
			*cursor++=scans[0]->plane_matches[j].ref->normal(1);
			*cursor++=scans[0]->plane_matches[j].ref->normal(2);
			*cursor++=scans[0]->plane_matches[j].ref->d;
//			fp<<scans[0]->plane_matches[j].ref->sqrt_cov_inv<<std::endl;
			memcpy(cursor_info,scans[0]->plane_matches[j].ref->sqrt_cov_inv.data(),16*sizeof(double));
			...
			{ // for each plane observation;
				...
				for(size_t k=0;k<plane_map_.size();k++)
				{
					if(plane_map_[k]->isObserved(scans[i]->plane_matches[j].ref))
					{
						...
						*cursor_camera++=i+1;
						*cursor++=scans[i]->plane_matches[j].cur->normal(0);
						*cursor++=scans[i]->plane_matches[j].cur->normal(1);
						*cursor++=scans[i]->plane_matches[j].cur->normal(2);
						*cursor++=scans[i]->plane_matches[j].cur->d;
//						fp<<scans[i]->plane_matches[j].cur->sqrt_cov_inv<<std::endl;
						memcpy(cursor_info,scans[i]->plane_matches[j].cur->sqrt_cov_inv.data(),16*sizeof(double));
//						fp<<"\t"<<"sqrt_information "<<std::endl
//						  <<"\t"<<cursor_info[0]<<"\t" <<cursor_info[1]<<"\t" <<cursor_info[2]<<"\t" <<cursor_info[3]<<std::endl    
//						  <<"\t"<<cursor_info[4]<<"\t" <<cursor_info[5]<<"\t" <<cursor_info[6]<<"\t" <<cursor_info[7]<<std::endl    
//						  <<"\t"<<cursor_info[8]<<"\t" <<cursor_info[9]<<"\t" <<cursor_info[10]<<"\t"<<cursor_info[11]<<std::endl  
...
//			fp<<scans[0]->line_matches[j].ref->sqrt_cov_inv<<std::endl;
			memcpy(cursor_info,scans[0]->line_matches[j].ref->sqrt_cov_inv.data(),36*sizeof(double));
//			fp<<"\t"<<"sqrt_information "<<std::endl
...
//						fp<<scans[i]->line_matches[j].cur->sqrt_cov_inv<<std::endl;
						memcpy(cursor_info,scans[i]->line_matches[j].cur->sqrt_cov_inv.data(),36*sizeof(double));
//						fp<<"\t"<<"sqrt_information "<<std::endl
//						  <<"\t"<<cursor_info[0]<<"\t"<<cursor_info[1]<<"\t"<<cursor_info[2]
//						  <<"\t"<<cursor_info[3]<<"\t"<<cursor_info[4]<<"\t"<<cursor_info[5]<<std::endl    
//						  <<"\t"<<cursor_info[6]<<"\t"<<cursor_info[7]<<"\t" <<cursor_info[8]
...
//		fp<<"v_update="<<v_update.transpose()<<std::endl;
	...
	bool MotionEstimation_Line::alignOccludingLines(Scan *scan, Transform &Tcr)
	{
		fp.open("motion_estimation_line.txt",std::ios::app);
		if(debug) 
...
//			Tcr.t.setZero();
	...
	bool MotionEstimation_Line::alignScans(Scan *scan, Map *map, Transform &Tcr, int num_frames,
							boost::shared_ptr<pcl::visualization::PCLVisualizer> vis)
	{
		fp.open("motion_estimation_line.txt",std::ios::app);
		if(debug) 
			fp<<std::endl<<"alignScans*******************************************"<<std::endl;

//		int num_frames=3;
		if(map->cameras.size()<num_frames) num_frames=map->cameras.size();

		plane_line_ba=new PlaneLineBA(map,num_frames,use_plane_,use_line_,use_shadow_);
		ceres::Problem problem;
		buildProblem(plane_line_ba,&problem);
		fp<<std::fixed<<scan->time_stamp
		  <<"\t"<<plane_line_ba->num_cameras()<<"\t"<<plane_line_ba->num_planes()<<"\t"<<plane_line_ba->num_lines()
		  <<"\t"<<plane_line_ba->num_observ_planes()<<"\t"<<plane_line_ba->num_observ_lines()
...
//					Eigen::Matrix<double,1,1> tmp=delta.transpose()*(*it)->sqrt_cov_inv*delta;
//					double dist=tmp(0,0);
//					if(dist<min)
//					{
//						min=dist;
...
//		}
	...
	void MotionEstimation_Line::buildProblem(PlaneLineBA* plane_line_ba, ceres::Problem* problem)
	{
...
//			information.setIdentity();
		...
		for(size_t i=0;i<plane_line_ba->num_cameras();i++)
		{
			...
			if(i<plane_line_ba->num_cameras()-1)
				problem->SetParameterBlockConstant(cameras+camera_block_size*i);
		}
		...
		for(size_t i=0;i<plane_line_ba->num_planes();i++)
		{
			...
			if(!use_line_residual_) problem->SetParameterBlockConstant(planes+plane_block_size*i);
//			problem->SetParameterBlockConstant(planes+plane_block_size*i);
		}
		...
		for(size_t i=0;i<plane_line_ba->num_lines();i++)
		{
			...
			if(!use_line_residual_) problem->SetParameterBlockConstant(lines+line_block_size*i);
//			problem->SetParameterBlockConstant(lines+line_block_size*i);
		}
		if(debug) fp<<"*********************************************************"<<std::endl<<std::endl;
	}

	Vector6d MotionEstimation_Line::computeLineShadow(Vector6d l, Eigen::Vector4d pi, Transform Tcg)
	{
